...
public class LightMixedLoadActivity extends AppCompatActivity implements Choreographer.FrameCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null && intent.hasExtra(SoftwareRenderMainActivity.EXTRA_LOAD_TYPE)) {
            mLoadType = intent.getIntExtra(SoftwareRenderMainActivity.EXTRA_LOAD_TYPE, LoadType.LIGHT_MIXED);
        }
    ...
    public void doFrame(long frameTimeNanos) {
        if (mIsTaskSchedulingEnabled && mIsScrolling) {
            ...
            mHandler.post(() -> { if (mIsScrolling) { mLoadSimulator.executeBetweenFrameLoad(mLoadType, "LightMixedLoad_betweenFrame"); } });
