...
public class MediumLoadActivity extends AppCompatActivity {
    ...
    private void initRecyclerView() {
        ...
        List<FriendCircleBean> data = PerformanceDataCenter.getInstance().generateDataForLoadType(PerformanceFriendCircleAdapter.LOAD_TYPE_MEDIUM);
        adapter.setFriendCircleBeans(data);
        Log.d(TAG, "initRecyclerView: 加载中负载类型的数据, 数据条数 = " + (data != null ? data.size() : 0));
