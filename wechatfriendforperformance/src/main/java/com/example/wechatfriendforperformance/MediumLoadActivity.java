...
/**
 * 中等负载Activity，每一帧的计算量中等，滑动过程中对CPU和GPU有一定压力
 */
public class MediumLoadActivity extends AppCompatActivity implements OnPraiseOrCommentClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 创建图片加载器，从本地资源加载图片 - 匹配原始项目的逻辑
        ImageLoader<String> mImageLoader = new ImageLoader<String>() {
            @Override
            public void loadImage(ImageView imageView, String imageUrl) {
                try {
                    ...
                    if (resourceId != 0) {
                        Glide.with(MediumLoadActivity.this)
                            .load(resourceId)
                            .centerCrop()
                            .into(imageView);
                    } else {
        ...
        mFriendCircleAdapter = new PerformanceFriendCircleAdapter(
            this, recyclerView, mImageLoader, PerformanceFriendCircleAdapter.LOAD_TYPE_MEDIUM);
