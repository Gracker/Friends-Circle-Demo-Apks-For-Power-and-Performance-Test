...
public class HeavyMixedLoadWebViewActivity extends BaseFriendCircleWebViewActivity {
    ...
    protected void performLoadTask() {
        Log.d(TAG, "混合重负载模式 - 等待 fling 后执行负载");
    }

    @Override
    protected void executeFlingLoad() {
        if (mLoadSimulator != null) {
            mLoadSimulator.executeDoFrameLoad(mLoadType, "HeavyMixedWV_fling_doFrame");
            mLoadSimulator.executeBetweenFrameLoad(mLoadType, "HeavyMixedWV_fling_betweenFrame");
