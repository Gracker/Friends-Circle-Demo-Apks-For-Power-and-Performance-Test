...
public class MediumLoadBetweenFramesActivity extends AppCompatActivity implements Choreographer.FrameCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null && intent.hasExtra(PerformanceMainActivity.EXTRA_LOAD_TYPE)) {
            mLoadType = intent.getIntExtra(PerformanceMainActivity.EXTRA_LOAD_TYPE, LoadType.MEDIUM_BETWEEN_FRAMES);
        }
    ...
    public void doFrame(long frameTimeNanos) {
        if (mIsBetweenFrameLoadEnabled && mIsScrolling) {
            ...
            if (mFrameCount >= mNextTriggerFrame) {
                mHandler.post(() -> { if (mIsScrolling) { mLoadSimulator.executePureBetweenFrameLoad(mLoadType, "MediumBetweenFrames_load"); } });
                mFrameCount = 0;
                mNextTriggerFrame = getNextFrameInterval();
            }
