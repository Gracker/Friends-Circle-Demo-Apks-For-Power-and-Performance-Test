...
import static com.example.wechatfriendforvideo.VideoConstants.*;
...
public class VideoDataCenter {
    ...
    private static final String TAG = "VideoDataCenter";
    ...
    private List<FriendCircleBean> generateFriendCircleBeans(int loadType) {
        ...
        for (int i = 0; i < FRIEND_CIRCLE_COUNT; i++) {
            ...
            // 新增：伪随机设置视频或图片
            // 使用固定的规则：每5个item中有1个视频
            if (i % 5 == 2) {
                // 设置为视频类型
                int[] videoResources = {R.raw.zwift_1, R.raw.zwift_2, R.raw.zwift_3, R.raw.zwift_4,
                                        R.raw.zwift_5, R.raw.zwift_6, R.raw.zwift_7, R.raw.zwift_8};
                int videoIndex = (i / 5) % videoResources.length;
                friendCircleBean.setVideoResId(videoResources[videoIndex]);
            } else if (i % 2 != 0) {
    ...
    private List<CommentBean> generateCommentBeans(int position, int loadType) {
        ...
        for (int i = 0; i < commentCount; i++) {
            ...
            // 评论内容
            String commentContent = VideoConstants.COMMENT_CONTENTS[
                    (position + i) % VideoConstants.COMMENT_CONTENTS.length];
