...
import static com.example.wechatfriendforcustomscroller.PerformanceConstants.*;

/**
 * Performance test data center, generate fixed test data
 */
public class CustomScrollDataGenerator {

    private static final String TAG = "CustomScrollDataGenerator";
    private static final int FRIEND_CIRCLE_COUNT = 100; // 固定生成100条朋友圈数据

    private static final CustomScrollDataGenerator instance = new CustomScrollDataGenerator();
    ...
    private CustomScrollDataGenerator() {
        // Initialization operations, if needed
    }
    
    public static CustomScrollDataGenerator getInstance() {
        return instance;
    }
    ...
    public List<FriendCircleBean> getFriendCircleBeans() {
        return getFriendCircleBeans(LoadProfile.LOAD_TYPE_LIGHT);
    }
    ...
    public List<FriendCircleBean> getFriendCircleBeans(int loadType) {
        ...
        switch (loadType) {
            ...
            default:
                if (cachedLightLoadFriendCircleBeans == null) {
                    cachedLightLoadFriendCircleBeans = generateFriendCircleBeans(LoadProfile.LOAD_TYPE_LIGHT);
                } else {
                }
                return cachedLightLoadFriendCircleBeans;
        }
