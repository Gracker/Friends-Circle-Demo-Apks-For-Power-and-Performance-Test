...
import static com.example.wechatfriendforsoftwarerender.SoftwareRenderConstants.*;

/**
 * Performance test data center, generate fixed test data
 */
public class SoftwareRenderDataCenter {

    private static final String TAG = "SoftwareRenderDataCenter";
    private static final int FRIEND_CIRCLE_COUNT = 100; // 固定生成100条朋友圈数据

    private static final SoftwareRenderDataCenter instance = new SoftwareRenderDataCenter();
    ...
    private SoftwareRenderDataCenter() {
        // Initialization operations, if needed
    }
    
    public static SoftwareRenderDataCenter getInstance() {
        return instance;
    }
    ...
    public List<FriendCircleBean> getFriendCircleBeans() {
        return getFriendCircleBeans(SoftwareRenderFriendCircleAdapter.LOAD_TYPE_LIGHT);
    }
    ...
    public List<FriendCircleBean> getFriendCircleBeans(int loadType) {
        ...
        switch (loadType) {
            ...
            default:
                if (cachedLightLoadFriendCircleBeans == null) {
                    cachedLightLoadFriendCircleBeans = generateFriendCircleBeans(SoftwareRenderFriendCircleAdapter.LOAD_TYPE_LIGHT);
                } else {
                }
                return cachedLightLoadFriendCircleBeans;
        }
    ...
    private List<CommentBean> generateCommentBeans(int position, int loadType) {
        ...
        for (int i = 0; i < commentCount; i++) {
            ...
            // 评论内容
            String commentContent = SoftwareRenderConstants.COMMENT_CONTENTS[
                    (position + i) % SoftwareRenderConstants.COMMENT_CONTENTS.length];
