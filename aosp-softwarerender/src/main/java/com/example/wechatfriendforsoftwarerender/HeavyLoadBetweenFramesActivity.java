...
public class HeavyLoadBetweenFramesActivity extends AppCompatActivity implements Choreographer.FrameCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null && intent.hasExtra(SoftwareRenderMainActivity.EXTRA_LOAD_TYPE)) {
            mLoadType = intent.getIntExtra(SoftwareRenderMainActivity.EXTRA_LOAD_TYPE, LoadType.HEAVY_BETWEEN_FRAMES);
        }
    ...
    public void doFrame(long frameTimeNanos) {
        if (mIsBetweenFrameLoadEnabled && mIsScrolling) {
            ...
            if (mFrameCount >= mNextTriggerFrame) {
                mHandler.post(() -> {
                    if (mIsScrolling) {
                        mLoadSimulator.executePureBetweenFrameLoad(mLoadType, "HeavyBetweenFrames_load");
                    }
                });
                mFrameCount = 0;
                mNextTriggerFrame = getNextFrameInterval();
